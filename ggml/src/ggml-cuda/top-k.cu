...
#if defined(GGML_CUDA_USE_CUB)  // CUB_TOP_K_AVAILABLE
    // Serial top-k launches one pass per row. For big multi-row inputs (indexer top-k at prefill)
    // the parallel radix kernel wins despite reading the data more times: the serial loop leaves the GPU idle.
    // Kill-switch LLAMA_CUDA_TOPK_RADIX=0 forces the serial path for A/B; unset = enabled.
